...
RELATIVE_STRENGTH_PROMPT = """全市场相对强弱盘：不预设中概互联网或美股大型科技优先，从整个 watchlist 的 trade 标的里做 1-3 个月波段机会比较。BUY 前必须横向比较至少 3 个不同行业候选，说明为什么当前标的相对更强；如果最终仍选择中概互联网或美股大型科技，必须明确证明它优于非核心科技候选，而不是因为熟悉度或新闻曝光更高。证据不足时 HOLD。"""

QUALITY_BALANCED_PROMPT = """质量均衡盘：刻意降低中概互联网和美股大型科技核心标的的默认优先级，优先研究消费、医疗、支付、工业、能源、金融、基础设施、非互联网软件等更分散的质量/现金流资产。目标是观察更均衡行业暴露能否降低回撤并提供不同机会来源。只有当中概互联网或美股大型科技的风险收益显著优于其它行业时才可买入，并必须写明比较依据。"""
...
EXPERIMENTS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "apply_mode": "auto",
        "ai_loop_enabled": True,
        "strategy_profile": "growth_aggressive",
        "tags": ["实验矩阵", "30万HKD", "全市场强弱", "非默认核心偏好"],
        "prompt": RELATIVE_STRENGTH_PROMPT,
        "risk": risk(100_000, 12_800, 3, 240),
        "hypothesis": "E30 检验去掉中概互联网/美股大型科技默认优先级后，AI 是否能从全池发现更强的跨行业机会，而不是被熟悉资产吸走注意力。",
        "success_metric": "比较 E30 vs C30/D30 的收益、回撤、行业分布和标的集中度；重点看是否降低阿里/腾讯/美股大科技集中度。",
        "expected_regime": "跨行业相对强弱环境；不假设中概互联网或美股大型科技一定是主线。",
    },
    {
        "name": "F30 质量均衡",
        "cash": 300_000,
        "apply_mode": "auto",
        "ai_loop_enabled": True,
        "strategy_profile": "defensive_cashflow",
        "tags": ["实验矩阵", "30万HKD", "质量均衡", "非核心科技"],
        "prompt": QUALITY_BALANCED_PROMPT,
        "risk": risk(100_000, 12_800, 2, 1_440),
        "hypothesis": "F30 检验更分散的质量/现金流行业暴露能否降低回撤，并提供不同于中概互联网与美股大型科技的收益来源。",
        "success_metric": "比较 F30 vs C30/E30 的收益、最大回撤、交易次数、行业分散度和新闻触发类型。",
        "expected_regime": "跨行业质量资产环境；目标是更稳的回撤控制和不同收益来源。",
    },
    {
        "name": "C50 波段纪律",
        "cash": 500_000,
...
def _upsert_experiment(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    hypothesis = {
        ...
        "expected_regime": spec.get("expected_regime", "中概互联网与美股科技 1-3 个月波段/新闻催化环境。"),
