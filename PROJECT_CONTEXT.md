...
>
> 当前系统总览、9 个实验盘、新闻维度到因子层路线和观察纪律记录在 `SYSTEM_OVERVIEW.md`。
>
...
- **阶段 2：复盘与策略评估层 MVP 已完成。**
- 当前系统已经具备“多模拟盘上下文 -> 行情/新闻/持仓输入 -> Gemini 多角色研究简报 -> AI 决策日志 -> observe/manual/auto 应用 -> 本地流水/富途成交回写 -> 收益曲线/决策追踪/执行质量/AB 对比/人工复盘”的第一版闭环。
- 这个阶段不是照搬 `tauricresearch/tradingagents`，而是在现有 Futu OpenD、autoNews、本地组合账本和 Web 工作区上自研轻量版研究小组决策结构。
- 下一阶段不应继续堆 agent 角色，应开始让阶段 2 数据跑一段时间，并围绕真实复盘结果优化策略、prompt 和风控。
...
6. 支持策略档案/策略标签、新闻因子效果、人工复盘标签和交易来源归因。
...
当前已录入的真实仓位镜像（`l r s`，active，本地组合 id `33192d8456`）：

- 腾讯控股 `HK.00700`：持有 100 股，成本 429.8 HKD，来自 2026-05-29 买入。
- 阿里巴巴-W `HK.09988`：持有 300 股，成本 125.0333 HKD，来自 2026-03-05、2026-04-09、2026-05-21 三次买入。
- 拼多多 `US.PDD`：持有 20 股，成本 89.115 USD，来自 2026-05-14 买入 10 股、2026-05-28 买入 10 股。
- 微软 `US.MSFT`：2026-02-25、2026-03-19、2026-03-25 累计买入 15 股；2026-05-28 已 425 USD 卖出 15 股，当前不持有。后续应做“卖出后如果继续持有”的反事实追踪。

截图提取的 2026 年交易流水（未完整入 `trades`，因为真实现金余额/资金流尚未确认，当前只录入持仓快照避免污染 NAV）：

| 日期 | 账户 | 标的 | 方向 | 数量 | 价格 |
| --- | --- | --- | --- | ---: | ---: |
| 2026-02-25 | 美股融资账户 | `US.MSFT` | 买入 | 5 | 396.96 |
| 2026-02-25 | 美股融资账户 | `US.PDD` | 买入 | 10 | 106.9 |
| 2026-03-05 | 港股融资账户 | `HK.09988` | 买入 | 100 | 126 |
| 2026-03-19 | 美股融资账户 | `US.MSFT` | 买入 | 5 | 390 |
| 2026-03-23 | 美股融资账户 | `US.PDD` | 买入 | 5 | 96.26 |
| 2026-03-25 | 美股融资账户 | `US.PDD` | 卖出 | 15 | 106.63 |
| 2026-03-25 | 美股融资账户 | `US.MSFT` | 买入 | 5 | 373.5 |
| 2026-04-09 | 港股融资账户 | `HK.09988` | 买入 | 100 | 123 |
| 2026-04-22 | 美股融资账户 | `US.PDD` | 买入 | 5 | 99.5 |
| 2026-05-13 | 美股融资账户 | `US.PDD` | 卖出 | 5 | 99.96 |
| 2026-05-14 | 美股融资账户 | `US.PDD` | 买入 | 10 | 96.23 |
| 2026-05-21 | 港股融资账户 | `HK.09988` | 买入 | 100 | 126.1 |
| 2026-05-28 | 美股融资账户 | `US.PDD` | 买入 | 10 | 82 |
| 2026-05-28 | 美股融资账户 | `US.MSFT` | 卖出 | 15 | 425 |
| 2026-05-29 | 港股融资账户 | `HK.00700` | 买入 | 100 | 429.8 |
...
- `ai-loop` 每 30 分钟运行一次。
- 自动循环会先判断市场时段：常规交易时段正常扫描和应用，开盘前准备窗口只生成计划，闭市/周末跳过。
- 会遍历所有本地模拟盘。
- 每个模拟盘分别调用 Gemini 决策。
...
- 账本层会拒绝常规交易时段外的 AI/manual 本地成交，`user_trade` 历史补录和 `futu_sync` 成交回写例外。
...
web/app.js                       前端交互逻辑
web/app.css                      前端样式
data/watchlist.default.json      分层学习观察池（core/learning/opportunity/macro）
...
- 2026-06-11 15:07 已加入 FX 来源追踪：系统会尝试富途 OpenD FX 快照；服务器实测 `FX.USDHKD` / `FX.USDCNH` 返回“不支持的行情市场”，底层 `SecurityType_Forex` 静态列表为空，因此会自动回退第三方中间价，也支持在组合页录入券商校准 HKD 汇率，并在前端、AI 决策上下文和成交流水里标记具体 FX 来源。
...
- 2026-06-11 阶段 2 第一波已上线：`/api/evaluation` 返回组合估值、每个模拟盘收益曲线、AI 决策后 1/3/7 天追踪、方向胜率和平均方向收益；前端新增 `评估` 页。
- 2026-06-11 收益曲线图例已可点击，用于控制对应模拟盘曲线显示或隐藏。
- 2026-06-11 评估 API 已增加交易归因统计，把 AI 自动应用、AI 手动应用、本人交易、富途成交回写按笔数、成交额、已实现盈亏拆开。
- 2026-06-11 阶段 2 MVP 已补齐：评估 API 会写入 `data/state/nav_history.json` 作为连续 NAV 快照；返回富途执行质量、AB Test 对比、策略标签表现、新闻因子效果、人工复盘汇总；前端 `评估` 页新增“阶段二复盘”区块；AI 决策详情页可保存人工复盘标签。
- 2026-06-11 模拟盘设置新增 `strategy_profile` 和 `strategy_tags`，会进入 AI 决策上下文和决策日志，用于后续按短线波段、长期持有、激进成长、防守现金流、新闻驱动等策略拆分复盘。
- 2026-06-11 组合设置继续扩展为实验说明书：每个组合可记录 `strategy_hypothesis`、`prompt_template` 和 `risk_overrides`，auto 决策会使用组合自己的有效风控；候选筛选新增新闻/异动/持仓触发器，`macro` 锚点只进入上下文不参与买卖候选。
- 2026-06-11 根据用户盈立证券佣金截图，把本地费用模型从粗略 bps 改为 `usmart_standard_commission_2026_screenshot_v1`：港股计入佣金/平台费/港交所与监管代收费/印花税，美股计入平台费/交收费/CAT/卖出侧 SEC 与 TAF，A 股通计入佣金/平台费/经手费/证管费/过户费/登记过户费/卖出印花税；交易流水保留 `fee_details`。
- 2026-06-11 已接入交易时段闸门：`ai-loop` 在闭市/周末跳过，开盘前窗口只扫描不成交；账本层拒绝常规交易时段外的 AI/manual 本地成交，避免盘前盘后或陈旧报价污染复盘数据。
- 2026-06-11 已加入组合级券商校准 FX 和第三方自动 FX：系统优先级为 `broker_manual_fx_to_hkd`（组合页手动校准）> `futu_opend_fx_snapshot` > `third_party_fx_to_hkd`（Frankfurter 主源，open.er-api 备用）> `local_default_fx_to_hkd`；组合显示、NAV 评估、风控、AI 决策上下文和后续交易流水都会使用同一有效汇率口径。

阶段 2 后续优化方向：

- 让系统运行一段时间，积累足够的 NAV 快照、成交和 1/3/7 天追踪样本。
- 后续可接更完整的历史 K 线，把 1/3/7 天表现从“首次到期后可得快照”升级为严格收盘价口径。
- 根据复盘结果进入阶段 3：策略/prompt/风控迭代，而不是盲目增加 agent 角色。

阶段 2 现在已经具备可用 MVP，优先级转为积累样本和解释表现。

阶段 3 启动节奏：

- 2026-06-11 阶段 2 刚上线后，先不要急着开发阶段 3；阶段 3 需要阶段 2 的真实复盘数据作为输入，否则容易做成空架子。
- 从 2026-06-11 起保持服务器和自动扫描/决策流程正常运行；第一批 1D 结果预计 2026-06-12 出现，3D 结果预计 2026-06-14 出现，7D 结果预计 2026-06-18 开始有意义。
- 这段时间用户主要做三件事：每天查看 `评估` 页的收益曲线、AI 决策后表现、AB Test 和新闻因子效果；对关键 AI 决策保存人工复盘标签和备注；观察富途执行质量/同步错误。
- 等第一批 7D 样本沉淀后，再进入阶段 3：策略版本、prompt 版本、风控参数版本追踪，并根据复盘结果生成下一轮策略/prompt/风控调整建议。
...
1. 决策日志固化复盘基线：决策时间、模拟盘 ID、组合净值、现金、持仓市值、行情快照、新闻信号、AI action/rating/position_action、application 状态。
2. 计算每条决策后的 1 天 / 3 天 / 7 天收益表现，当前先使用首次到期后可得 OpenD 快照。
3. 评估 API 返回模拟盘绩效：总资产、收益率曲线、最大回撤、胜率、盈亏比、交易次数、交易频率、换手。
4. 前端 `评估` 页按模拟盘和时间范围查看表现，并支持收益曲线图例隐藏/显示。
5. AI 自动应用、AI 手动应用、本人交易、富途成交回写已分开统计。
6. 已支持策略档案/标签、富途执行质量、新闻因子效果和人工复盘标签。
